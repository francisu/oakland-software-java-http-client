...
public class PipelineTester //implements CheckResults
 {
    ...
    public PipelineTester(String url, int iterations, int pipeliningOptions,
    ...
    public synchronized void setFailType(int failType, Exception ex,
        int responseCode) {
        ...
        if (_ignoreFailType != failType) {
            System.out.println("failure " + failType + " respCode: " +
                responseCode + " exception: " + ex);
        }
    ...
    public boolean runTestMt() throws Exception {
        ...
        for (int i = 0; i < _iterations; i++) {
            ...
            String urlString = _url + (_url.endsWith("&") ? "" : "?") +
                COUNT_PROP + "=" + Integer.toString(i + 1) + "&threadName=" +
            ...
            urlCon.setRequestProperty("threadName",
                Thread.currentThread().getName());
        ...
        if (_showStats) {
            System.out.println("Number of connections for test: " +
                _iterations);
        }
    ...
    public boolean checkCounts() {
        ...
        for (int i = 1; i <= _iterations; i++) {
            ...
            if (_responseCounts[i] != 1) {
                System.out.println("Response count incorrect for req: " + i +
                    " count is: " + _responseCounts[i]);
                fail = true;
            }
        }

        if (_iterations != totalResponses) {
            System.out.println("totalResponses mismatch: " + _iterations +
                " read counts: " + totalResponses);
            fail = true;
        }
    ...
    public boolean checkResults() {
        ...
        if (_iterations != _readCountOk) {
            System.out.println("Iterations mismatch: " + _iterations +
                " read counts: " + _readCountOk);
            fail = true;
        }
    ...
    public class TestCallback implements Callback {
        public void writeRequest(HttpURLConnection urlCon, OutputStream os) {
            ...
            try {
                if (_streamingType == HttpTestBase.STREAM_RAW) {
                    ...
                    String outString = "PUT " + urlString + " HTTP/1.1\r\n" +
                        "Content-Length: " + _outputData.length() + "\r\n\r\n";
        ...
        public void error(HttpURLConnection urlCon, InputStream is, Exception ex) {
            try {
                if ((urlCon.getResponseCode() == _expectedResponse) &&
                        (ex == null)) {
                    synchronized (PipelineTester.this) {
                        _readCountOk++;
                    }
