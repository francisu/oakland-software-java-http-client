...
public class TestRedirect extends TestWebappBase {
    ...
    public void testRedirect(String method, int redirectCode)
        throws Exception {
        ...
        qs += ("to=" +
        URLEncoder.encode("http://" + HttpTestEnv.TOMCAT_HOST + ":" +
            HttpTestEnv.TOMCAT_PORT_1 + "/" + context + ParamServlet.NAME,
        ...
        if (!method.equals("GET")) {
            ...
            if (method.equals("PUT")) {
                os.write("This is data to be sent in the body of an HTTP PUT.".getBytes(
                        "ASCII"));
            }
        }
        ...
        assertTrue(checkReplyNoAssert(reply,
                "<title>Param Servlet: " + method + "</title>"));
    ...
    public void testRedirectHostPort(String hostRedir, int portRedir,
        boolean fail) throws Exception {
        assertTrue(java.net.HttpURLConnection.getFollowRedirects());

        URL url = new URL(_urlBase + RedirectServlet.NAME + "?to=" +
                URLEncoder.encode("http://" + hostRedir + ":" + portRedir +
                    "/" + context + ParamServlet.NAME, Util.DEFAULT_ENCODING));
        int response = 0;
        ...
        try {
            ...
            if (fail) {
                if (urlCon.usingProxy()) {
                    // Squid sends a 504, apache 2.x gives 404
                    if ((response != 502) && (response != 504) &&
                            (response != 404)) {
                        fail("Proxy and not correct response: " + response);
                    }
                } else {
    ...
    public void testRedirectHostPort() throws Exception {
        testRedirectHostPort(getServletIpAddress(), HttpTestEnv.TOMCAT_PORT_1,
            !FAIL);
    }
    ...
    public void testNoRedirect() throws Exception {
        ...
        URL url = new URL(_urlBase + RedirectServlet.NAME + "?to=" +
                URLEncoder.encode("http://" + HttpTestEnv.TOMCAT_HOST + ":" +
                    HttpTestEnv.TOMCAT_PORT_1 + "/" + context +
    ...
    public void testRedirectWithQueryString() throws Exception {
        String qs = URLEncoder.encode("http://" + HttpTestEnv.TOMCAT_HOST +
                ":" + HttpTestEnv.TOMCAT_PORT_1 + "/" + context +
    ...
    public void testRecursiveRedirect() throws Exception {
        String qs = "to=" +
            URLEncoder.encode("http://" + HttpTestEnv.TOMCAT_HOST + ":" +
                HttpTestEnv.TOMCAT_PORT_1 + "/" + context + "/params?foo=bar",
                Util.DEFAULT_ENCODING);

        for (int i = 0; i < 10; i++) {
            qs = "to=" +
                URLEncoder.encode("http://" + HttpTestEnv.TOMCAT_HOST + ":" +
                    HttpTestEnv.TOMCAT_PORT_1 + "/" + context + "/redirect?" +
    ...
    public void allTestMethods() throws Exception {
        if (false) {
            LogUtils.logFile("/home/francis/log4jredir" + _testAllName +
                ".txt");
        }
