...
public class TestMultiThread extends TestWebappBase {
    ...
    static {
        StringBuffer textBuf = new StringBuffer(TEXT_TIMES * HeaderServlet.TEXT_CONST.length());
    ...
    protected void threadMethod(String method, String urlSuffix,
        String servletName, boolean hasExtendedText) {
        try {
            URL url = new URL(_urlBase + urlSuffix);
            int response = 0;

            _log.debug(Thread.currentThread().getName() + " " + method + " " +
                urlSuffix + " " + servletName);
            ...
            boolean localPassed = checkReplyNoAssert(reply,
                    "<title>" + servletName + " Servlet: " + method +
                    "</title>");
    ...
    protected void threadsBase() throws Exception {
        ...
        if (com.oaklandsw.http.HttpURLConnection.getMaxConnectionsPerHost() == 2) {
            com.oaklandsw.http.HttpURLConnection.setMaxConnectionsPerHost(MAX_OPEN_PER_HOST);
        }
                        ...
                        public void run() {
                            try {
                                Thread.currentThread()
                                      .setName("TestMultiThread" + threadNum);
                                ...
                                for (int j = 0; j < _times; j++) {
                                    ...
                                    threadMethod(com.oaklandsw.http.HttpURLConnection.HTTP_METHOD_GET,
                                        RedirectServlet.NAME + "?to=" +
                                        URLEncoder.encode("http://" +
                                            HttpTestEnv.TOMCAT_HOST + ":" +
                                            HttpTestEnv.TOMCAT_PORT_1 + "/" +
                                            context + ParamServlet.NAME,
                                            Util.DEFAULT_ENCODING), "Param",
                                        false);
