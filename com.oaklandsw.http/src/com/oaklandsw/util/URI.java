...
public class URI implements Cloneable, Comparable, Serializable {
	 ...
	 * @exception URIException
	 */
	public URI(String sch, String ui, String h, int p, String pth, String q,
			String f) throws URIException {

		this(sch, (h == null) ? null : ((ui != null) ? ui + '@' : "") + h
				+ ((p != -1) ? ":" + p : ""), pth, q, f);
	}
	...
	static {
		...
		if (locale != null) {
			defaultDocumentCharsetByLocale = LocaleToCharsetMap
					.getCharset(locale);
			// set the default document charset
			_documentCharset = defaultDocumentCharsetByLocale;
		}
		// in order to support platform encoding
		try {
			defaultDocumentCharsetByPlatform = System
					.getProperty("file.encoding");
		} catch (SecurityException ignore) {
		}
		if (_documentCharset == null) {
			// set the default document charset
			_documentCharset = defaultDocumentCharsetByPlatform;
		}
	}
	...
	protected void parseUriReference(String original, boolean e)
		...
		if (0 <= at && at + 1 < length && tmp.charAt(at) == '?') {
			...
			_query = (e) ? tmp.substring(at + 1, next).toCharArray() : encode(
					tmp.substring(at + 1, next), allowed_query);
			at = next;
		}
		 ...
		 */
		if (0 <= at && at + 1 < length && tmp.charAt(at) == '#') {
			_fragment = (e) ? tmp.substring(at + 1).toCharArray() : encode(
					tmp.substring(at + 1), allowed_fragment);
		}
	...
	protected void parseAuthority(String original, boolean e)
		...
		if (next != -1) { // neither -1 and 0
							// each protocol extented from URI supports the
							// specific userinfo
			_userinfo = (e) ? original.substring(0, next).toCharArray()
		...
		if (next >= from) {
			...
			_host = (e) ? original.substring(from, next).toCharArray()
		...
		if (_is_reg_name) {
			...
			_authority = (e) ? original.toString().toCharArray() : encode(
					original.toString(), allowed_reg_name);
		} else {
