 ...
 * 
 * @version $Revision: 157457 $ $Date: 2005-03-14 12:23:16 -0800 (Mon, 14 Mar
 *          2005) $
 */
public class Cookie extends NameValuePair implements Serializable, Comparator
{
     ...
     *            <tt>null</tt> if the cookie expires at the end of the
     *            session
     * @param secure
     ...
     *            is expected to be a non-negative number. <tt>-1</tt>
     *            signifies that the cookie should never expire.
     * @param secure
     ...
     * <p>
     * When <tt>true</tt> the cookie should only be sent using a secure
     * protocol (https). This should only be set when the cookie's originating
    ...
    {
        return (cookieExpiryDate != null && cookieExpiryDate.getTime() <= System
                .currentTimeMillis());
    }
    ...
    {
        return (cookieExpiryDate != null && cookieExpiryDate.getTime() <= now
                .getTime());
    }
     ...
     * of the <tt>Set-Cookie</tt> header. This value is important for
     * generating the <tt>Cookie</tt> header because some cookie
     * specifications require that the <tt>Cookie</tt> header should only
     * include a domain attribute if the cookie's domain was specified in the
    ...
    public int hashCode()
    {
        int hash = Util.HASH_SEED;
        hash = Util.hashCode(hash, this.getName());
        hash = Util.hashCode(hash, this.cookieDomain);
        hash = Util.hashCode(hash, this.cookiePath);
        return hash;
    }
    ...
    public boolean equals(Object obj)
    {
        ...
        if (obj instanceof Cookie)
        {
            ...
            return StringUtils.equals(this.getName(), that.getName())
                && StringUtils.equals(this.cookieDomain, that.cookieDomain)
                && StringUtils.equals(this.cookiePath, that.cookiePath);
        }
        return false;
    }
     ...
     * specific Locales.
     */
    private static final RuleBasedCollator STRING_COLLATOR    = (RuleBasedCollator)Collator
